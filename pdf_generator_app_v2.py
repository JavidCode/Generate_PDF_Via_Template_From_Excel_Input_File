...
def generate_upc(product_id, barcode_format):
    barcode = generate(barcode_format, product_id, writer=ImageWriter())
    filename = f"upc_{product_id}.png"
    barcode.save(filename)
    return filename, str(barcode.get_fullcode())
...
def extract_data_from_pdf(pdf_path, consignor_id, field_mapping):
    ...
    for annotation in annotations:
        ...
        # Add UPC field
        if field_name.lower() == 'upc':
            upc_filename, upc_value = generate_upc(consignor_id + str(len(data['Size']) + 1), 'UPCA')
            data['UPC'].append(upc_filename)
...
def save_output_as_pdf(pdf_data, output_directory, consignor_id, index):
    ...
    # Draw the UPC on the PDF
    c.drawInlineImage(pdf_data['UPC'][0], 100, 710, width=150, height=50)

    # Iterate through Description fields
    for i in range(1, 10):
        description_key = f'Description{i}'
        if pdf_data.get(description_key):
            c.drawString(100, 690 - (20 * i), f"{description_key}: {pdf_data[description_key][0]}")
...
def main():
    ...
    consignor_id = input("Enter Consignor ID: ")
